...
> [!IMPORTANT]
> **本 fork 的市场状态：** 本现代化 fork 当前没有由 Xero-Team 运营的独立官方插件
> 市场。Dashboard 和后端的默认 registry 仍指向 AstrBot 上游服务，以兼容现有流程。
> 本文定义的是当前客户端、后端及自定义 registry 接受的 JSON 协议，不是在宣布
> Xero-Team 市场，也不表示上游服务由本 fork 运营或背书。
...
示例中的 `astrbot.app` 与 `AstrBotDevs` 地址用于说明当前上游兼容源，不是本 fork
运营的服务端点。实现者可以提供符合本文协议的自定义 registry。
...
1. 客户端通过运行时请求字段 `market_plugin_id` 向后端发送选中的 `plugin_id`，并同时携带 registry source。
...
3. 候选市场记录应该具有相同仓库身份。
4. 绑定请求应该使用运行时字段 `market_plugin_id` 传递选中的 `plugin_id`。
5. 绑定必须持久化选中的 registry source 和 `plugin_id`。
...
## 16. 当前 Dashboard / OpenAPI 互操作约束

当前分支的 dashboard / backend API 契约如下：

- 运行时安装请求和绑定请求可以携带 `market_plugin_id`；它只是请求字段，不得写入市场源 JSON。
- 已安装插件的 REST 资源使用路径参数 `{plugin_id}` 表示已安装插件的 name/path key。
- `GET /api/v1/plugins` 过滤时必须使用 `name` 查询参数；`plugin_id` 等旧查询参数会被拒绝。
- `POST /api/v1/plugins/update` 必须使用 `names` 数组；`plugin_ids` 等旧请求体字段会被拒绝。
