...
> [!IMPORTANT]
> **Marketplace status for this fork:** This modernized fork does not currently
> operate an independent official plugin marketplace under Xero-Team. The Dashboard
> and backend default registry still points to upstream AstrBot services for
> compatibility with existing workflows. This document defines the JSON protocol
> accepted by the current client, backend, and custom registries; it does not establish
> an Xero-Team marketplace or imply that this fork operates or endorses upstream
> services.
...
```json
{
  "$meta": {
    "schema_version": 1,
    "name": "AstrBot Upstream Plugin Market",
    "version": "2026-06-27",
...
The `astrbot.app` and `AstrBotDevs` URLs in this example identify the currently
compatible upstream source; they are not service endpoints operated by this fork.
Implementers may provide a custom registry that follows this protocol.
...
1. The client sends the selected `plugin_id` to the backend using the runtime request field `market_plugin_id`, together with the registry source.
...
4. Binding requests SHOULD send the selected `plugin_id` using the runtime field `market_plugin_id`.
5. Binding MUST persist the selected registry source and `plugin_id`.
...
## 16. Current Dashboard/OpenAPI Interop

The current dashboard/backend API contract on this branch is:

- Runtime install and bind requests MAY carry `market_plugin_id`; this is a request field only and MUST NOT be emitted by market source JSON.
- Installed-plugin REST resources use the path parameter `{plugin_id}` for the installed plugin name/path key.
- `GET /api/v1/plugins` MUST use the `name` query parameter when filtering; legacy query parameters such as `plugin_id` are rejected.
- `POST /api/v1/plugins/update` MUST use the `names` array; legacy body fields such as `plugin_ids` are rejected.
